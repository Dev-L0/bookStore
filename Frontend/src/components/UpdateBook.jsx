...
const UpdateBook = () => {
  ...
  const{updateBook, getBookById,selectedBook, isLoading, error} = useBookStore();
  ...
  const handleSubmit = async (e)=>{
    e.preventDefault();
    try{
      await updateBook(id, name, price, category, title, image);
      toast.success("Succesfully updated a book!");
      setTimeout(()=>{navigate('/all-books');}, 1000);
    ...
    <div className="flex items-center justify-center h-screen text-sm mt-10">
      <div className="dark:bg-slate-400">
        <form onSubmit={handleSubmit} className="flex flex-col items-center">
          <h3 className="font-bold text-2xl">Update Book</h3>
          <div>
            {/* book name */}
